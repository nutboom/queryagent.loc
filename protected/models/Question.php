 ...
 * @property integer $scaled_size
 * @property integer $is_class
 * @property integer $is_not_required
 */
class Question extends CActiveRecord
{
    ...
    const TYPE_SEMICLOSE_MULTIPLE = "semiclose_multiple";
	...
	public function rules()
	{
		...
		return array(
                        array('text', 'required'),
			array('group_id, orderby, scaled_size, is_class, is_not_required', 'numerical', 'integerOnly'=>true),
			...
			array('id, group_id, text, type, orderby, scaled_size, is_class, is_not_required', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
                    'answers' => array(self::HAS_MANY, 'Answer', 'question_id','order'=>'answers.orderby','index'=>'id'),
                    'respondentAnswer' => array(self::HAS_MANY, 'ApplicationAnswer', 'question_id'),
                    'pictures' => array(self::HAS_MANY, 'QuestionMedia', 'question_id'),
                    'group' => array(self::BELONGS_TO, 'GroupQuestions', 'group_id'),
                    'groupConditionQuestion' => array(self::HAS_ONE, 'GroupQuestions', 'condition_question_id'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			...
			'is_not_required' => 'Необязательный для ответа',
		);
	}
        ...
        public static function itemAlias($type,$code=NULL) {
            $_items = array(
                'TypeQuestion' => array(
                    ...
                    self::TYPE_SEMICLOSE_MULTIPLE => "Полузакрытый с множественным выбором",
	...
	public function search()
	{
		...
		$criteria->compare('is_not_required',$this->is_not_required);
	...
	public function checkSetAnswers($attribute, $params)
	{
            if(!$this->hasErrors())  // we only want to authenticate when no input errors
            {
                switch ($this->type){
                    ...
                    case self::TYPE_SEMICLOSE_MULTIPLE:
         ...
         */
        public function getStatsAnswers($condition = '', $params = array(), $quiz_id = false, $omi_aud_id = false) {
            $answersAppl = array();
            $statsQ = array();
            $omi_condition = '';
            if($quiz_id && $omi_aud_id)
                $omi_condition = ' AND application_id IN (SELECT id FROM tbl_applications WHERE quiz_id='.$quiz_id.' AND state="close" AND omi_aud_id='.$omi_aud_id.") ";
            $sql = "SELECT answer_id, count(question_id) AS count_answer, avg(answer_text) AS sum_answer_text
FROM
(
    SELECT application_id, question_id, answer_id, answer_text
	FROM {{application_answers}} qa
	WHERE qa.question_id=:question".($condition ? ' AND '.$condition : '').$omi_condition."
	GROUP BY application_id, question_id, answer_id
) a
GROUP BY answer_id";
            $answersAppl = Yii::app()->db->createCommand($sql)->queryAll(true, array_merge(array(':question'=>$this->id), $params));
