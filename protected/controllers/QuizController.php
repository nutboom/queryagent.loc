...
class QuizController extends Controller
{
	...
	public function actionPrecreate($type) {
        ...
        */
		...
		if ($count == 0) $this->redirect(array('quiz/create','type'=>$type));
        else
	...
	public function actionCreate($type) {
		...
		if(isset($_POST['Quiz'])) {
			...
			if($model->save()) {
				$this->redirect(array($type."/".$model->quiz_id."/StructureQuiz"));
    		}
		}
	...
	public function actionLaunch($type, $id) {
		$model=$this->loadModel($id);
		$omiAud = OmiTargetAudience::model()->findAllByAttributes(array('quiz_id'=>$id));
		...
		if(isset($_POST['Quiz'])) {
	  		if ($model->state == Quiz::STATE_EDIT) {
	  			if((count($model->audience) > 0 || count($omiAud)>0) && $model->countQuestions() > 0) {
	  				$model->attributes=$_POST['Quiz'];

				  	// if quiz use respondents in all base set moderation status
				  	$base = TargetAudience::haveBaseAudience($model);
				  	$omiBase = OmiTargetAudience::haveOmiAudience($id);
				  	if ($base == true || $omiBase == true) {
						...
						UserModule::sendMail("vadimshavlukevich@yandex.ru", "Параметры целевой аудитории OMI", OmiTargetAudience::getAudienceParams($model->quiz_id));
						$user->balance = $user->balance - $omiTotalLimit*165;
						$user->save(false);
                ...
                elseif($model->by_link == 1  && $model->countQuestions() > 0)
                {
                    $model->attributes=$_POST['Quiz'];
                    $model->state = Quiz::STATE_WORK;
                    if($model->save()) {
    ...
    public function actionExcel($id){
        ...
        $model = $this->loadModel($id);
        $color = Branding::getTopColor();
        $logo = Branding::getLogo();
        $arrayForExport = ExportFiles::getArrayForExport($id);
...
// Подписываем лист
        ...
        $sheet->getStyle('A1')->getFill()->getStartColor()->setRGB($color);
...
');
        ...
        $svg = glob(Yii::app()->basePath."/../*.svg");
        array_map('unlink', $svg);
        exit();
    }

    public function getCountQuizStat($quizId, $hash){

        $bitly_clicks_count = Yii::app()->mybitly->getClicks("http://panel.queryagent.ru/?h=".$hash);
        $yandex_metrics = 0;

        $sql = "SELECT COUNT(*) qStart FROM tbl_applications WHERE quiz_id = :quizId";
        $result = Yii::app()->db->createCommand($sql)->queryAll(true, array(":quizId"=>$quizId));
        $data['start'] = $result[0]['qStart'];

        $sql = "SELECT COUNT(*) qClose FROM tbl_applications WHERE quiz_id = :quizId and state = 'close'";
        $result = Yii::app()->db->createCommand($sql)->queryAll(true, array(":quizId"=>$quizId));
        $data['close'] = $result[0]['qClose'];

        $sql = "SELECT COUNT(*) qAnonym FROM tbl_applications WHERE quiz_id = :quizId and respondent_id is null and state='close'";
        $result = Yii::app()->db->createCommand($sql)->queryAll(true, array(":quizId"=>$quizId));
        $data['anonym'] = $result[0]['qAnonym'];
        $data['open'] = $bitly_clicks_count + $yandex_metrics;

        return $data;

    }

    public function getYaMetrics($hash){
        include Yii::getPathOfAlias('ext').DIRECTORY_SEPARATOR.'yandex'.DIRECTORY_SEPARATOR.'YandexAPI.php';
        $app_id = '5d291dba25ae4e97b3a75cd9329d3a3c';
        $app_pass = 'e0ec80ae0667412485b3a688a5ea6d68';
        $api = new YandexAPI($app_id, $app_pass);
        if($api->LogIn('test@queryagent.ru', '123456')){
            $api->MakeQuery('/counters');

            $api->MakeQuery('/stat/content/entrance', array('id'=>'23332327', 'date1'=>'20120101', 'date2'=>'20150313'));
            $res = $api->result;
            for($i = 0; $i < sizeof($res['data']); ++$i){

                if (strpos($res['data'][$i]['url'], $hash) > 0)
                {
                    return $res['data'][$i]['visits'];
                }
            }
        }
        else{
            echo "WTF";return false;
        }
    }
}
