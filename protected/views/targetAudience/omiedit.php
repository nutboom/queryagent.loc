...
?>
    ...
    <div class="step-pane">
        ...
        <form action="omisaveaud" id="omi_form" method="post">
            ...
            <input type="hidden" name="audModel_id" value="<?=$audModel->id?>">
            ...
            <div class="aud_attr">
                ...
                <select name="citysize[]" multiple class="selectpicker" data-header="Выберите размер города" data-selected-text-format="count>2">
                    ...
                    {
                        $selected = in_array($value['id'], explode(',',$audModel->citysize))?'selected':'';
                        ?>
            ...
            <div class="aud_attr" id="pre_last_block">
                ...
                <select name="regions[]" multiple class="selectpicker" data-header="Выберите регион" data-selected-text-format="count>2" data-live-search="true" data-size="10">
                    ...
                    {
                        $selected = in_array($value['id'], explode(',',$audModel->region))?'selected':'';
                        ?>
                        <option <?=$selected?> value="<?=$value['id']?>"><?=$value['title']?></option>
                    <?
                    }
            ...
            <div class="aud_attr">
                Возраст:<br>
                от <input type="text" value="<?=$audModel->age_from?>" name="age_from" style="width: 80px; margin-right: 10px; margin-left: 5px;">   до <input type="text" value="<?=$audModel->age_to?>" name="age_to" style="width: 80px; margin-left: 5px;">
            </div>

            <div class="aud_attr">
                Пол:<br>
                <select name="sex" class="selectpicker" data-header="Выберите пол">
                    <option <?echo $audModel->sex==0?'selected':'';?> value="0">Любой</option>
                    <option <?echo $audModel->sex==1?'selected':'';?> value="1">Мужской</option>
                    <option <?echo $audModel->sex==2?'selected':'';?> value="2">Женский</option>
                </select>
            </div><br>
            ...
            <div class="aud_attr">
                ...
                <select name="education[]" multiple class="selectpicker" data-header="Выберите образование" data-selected-text-format="count>2">
                    ...
                    {
                        $selected = in_array($value['id'], explode(',',$audModel->education))?'selected':'';
                        ?>
                        <option <?=$selected?> value="<?=$value['id']?>"><?=$value['title']?></option>
                    <?
                    }
            ...
            <div class="aud_attr">
                ...
                <select name="jobsphere[]" multiple class="selectpicker" data-size="10" data-header="Выберите сферу деятельности" data-selected-text-format="count>2">
                    ...
                    {
                        $selected = in_array($value['id'], explode(',',$audModel->jobsphere))?'selected':'';
                        ?>
                        <option <?=$selected?> value="<?=$value['id']?>"><?=$value['title']?></option>
                    <?
                    }
            ...
            <div class="aud_attr">
                ...
                <select name="evaluation[]" multiple class="selectpicker" data-header="Выберите мат. положение" data-selected-text-format="count>2">
                    ...
                    {
                        $selected = in_array($value['id'], explode(',',$audModel->evaluation))?'selected':'';
                        ?>
                        <option <?=$selected?> value="<?=$value['id']?>"><?=$value['title']?></option>
                    <?
                    }
