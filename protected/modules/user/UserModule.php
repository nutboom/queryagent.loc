...
class UserModule extends CWebModule
{
	...
	public $activateUrl = array("/user/finance/pay");
	public $mainUrl = array("/site");
	public $thankyou = array("/thankyou");
	...
	public $tableClients = '{{clients}}';
	public $tableUsersClients = '{{users_clients}}';

    public $defaultScope = array(
            'with'=>array('profile','client'),
    );
	...
	static private $_marketer;
	static private $_client;
	static private $_managers;
	static private $_marketers;
	static private $_clients;
     ...
     * @return boolean
     */
    public static function isMarketer() {
        if(Yii::app()->user->isGuest)
            return false;
        else {
            if (!isset(self::$_marketer)) {
                if(self::user()->marketer)
                    self::$_marketer = true;
                else
                    self::$_marketer = false;
            }
            return self::$_marketer;
        }
    }
	...
	public static function isClient() {
		if(Yii::app()->user->isGuest)
			...
			if (!isset(self::$_client)) {
				if(!self::user()->superuser && !self::user()->manager && !self::user()->marketer)
					self::$_client = true;
				else
					self::$_client = false;
			}
	...
	/**
	 * Return manager.
	 * @return array managers names
	 */
	public static function getManagers() {
		if (!self::$_managers) {
			$managers = User::model()->active()->manager()->findAll();
			$return_name = array();
			foreach ($managers as $manager)
				array_push($return_name,$manager->username);
			self::$_managers = ($return_name)?$return_name:array('');
		}
     ...
     * @return array managers names
     */
    public static function getManagerMarketers() {
        if (!self::$_marketers) {
            $managers = User::model()->active()->manager()->marketer()->findAll();
            $return_name = array();
            foreach ($managers as $manager)
                array_push($return_name,$manager->username);
            self::$_marketers = ($return_name)?$return_name:array('');
        }
	 ...
	 * @return array client managers names
	 */
	public static function getClientUsers() {
		if (!self::$_clients) {
			$clients = User::model()->active()->viewClients()->findAll();
			$return_name = array();
			foreach ($clients as $client)
				array_push($return_name,$client->username);
			self::$_clients = ($return_name)?$return_name:array('');
		}
	...
	public static function user($id=0,$clearCache=false) {
		...
		if ($id) {
            if (!isset(self::$_users[$id])||$clearCache)
                self::$_users[$id] = User::model()->with(array('profile','client'))->findbyPk($id);
